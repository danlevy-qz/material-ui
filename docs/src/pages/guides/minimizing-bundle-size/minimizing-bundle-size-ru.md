...
Размер пакета Material-UI достаточно внушителен. Size snapshots are taken on every commit for every package and critical parts of those packages ([view the latest snapshot](/size-snapshot)). Combined with [dangerJS](https://danger.systems/js/) we can inspect [detailed bundle size changes](https://github.com/quizlet/material-ui/pull/14638#issuecomment-466658459) on every Pull Request.
